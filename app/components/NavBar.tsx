...
export default function NavBar() {
  ...
  return (
    <>
    {user?.id ? (
      <nav className="w-full bg-opacity-0 z-10">
        <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto px-12 py-6">
          <div className="w-3/4 flex flex-col justify-between">
            {!showNav && (
          <button onClick={() => handleNav()} className="inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-licorice rounded-lg hover:bg-peach focus:outline-none focus:ring-2 focus:ring-peach text-licorice" aria-controls="navbar-default" aria-expanded="false">
          ...
          {showNav && (
            ...
            <ul className="font-medium flex px-8 mx-16 rounded-lg w-full bg-white">
              ...
              <li>
                <Link href="/chat" className="block py-2 pl-3 pr-4 text-licorice rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:p-0 hover:bg-peach hover:text-white md:hover:bg-transparen">Chat</Link>
              </li>
              <li>
                <button onClick={handleSignOut} className="block py-2 pl-3 pr-4 text-licorice rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:p-0 hover:bg-peach hover:text-white md:hover:bg-transparen">Sign out</button>
              </li>
            </ul>
            </div>
          )}
