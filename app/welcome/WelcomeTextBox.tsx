...
export default function WelcomeTextBox() {
  ...
  const handleWelcome = async (username: string) => {
    try {
      ...
      toast.error("Error updating your name. Please try again.");
      throw error; // Re-throw to prevent continuing the flow
    }
  };
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      // Error already handled in handleWelcome
    }
  };
        ...
        <div className="flex flex-col p-1 m-2">
          ...
          >
            <input
              ...
              className="ring-peach rounded-lg ring-2 ring-inset m-3 p-2 shadow-md shadow-licorice/30 w-1/2 caret-peach text-md disabled:opacity-50"
            />
            <button
              type="submit"
              disabled={updateUser.isPending}
